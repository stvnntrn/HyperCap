...
const GamingCoins = () => {
  ...
  const getMarketCapRanking = () => {
    const sortedByMarketCap = [...gamingCoins].sort(
  ...
  const totalMarketCap = gamingCoins.reduce(
    (sum, coin) => sum + coin.marketCap,
    0
  );
  const totalVolume = gamingCoins.reduce((sum, coin) => sum + coin.volume, 0);

  // Get trending Gaming coins by volume
  const trendingGamingCoins = [...gamingCoins]
    .sort((a, b) => b.volume - a.volume)
    .slice(0, 3);

  // Get top gainers
  const topGainers = [...gamingCoins]
    .sort((a, b) => b.change24h - a.change24h)
    .slice(0, 3);

  // Get newly listed coins (using market cap as a proxy for listing time)
  const newlyListed = [...gamingCoins]
    ...
    return (
      <svg width={width} height={height} className="w-full">
        ...
        <path
          ...
          fill={
            isPositive ? "rgba(139, 92, 246, 0.1)" : "rgba(239, 68, 68, 0.1)"
          }
        />
      </svg>
    );
    ...
    <div className="container mx-auto bg-white text-gray-800 p-6 rounded-xl">
      <div className="mb-8">
        ...
        <h2 className="text-2xl font-bold mb-4 mt-8">
          Gaming Cryptocurrency Prices by Market Cap
        </h2>
