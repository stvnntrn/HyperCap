...
const LayerOneCoins = () => {
  ...
  const getMarketCapRanking = () => {
    const sortedByMarketCap = [...layerOneCoins].sort(
  ...
  const totalMarketCap = layerOneCoins.reduce(
    (sum, coin) => sum + coin.marketCap,
    0
  );
  const totalVolume = layerOneCoins.reduce((sum, coin) => sum + coin.volume, 0);

  // Get trending Layer 1 coins by volume
  const trendingLayerOneCoins = [...layerOneCoins]
    .sort((a, b) => b.volume - a.volume)
    .slice(0, 3);

  // Get top gainers
  const topGainers = [...layerOneCoins]
    .sort((a, b) => b.change24h - a.change24h)
    .slice(0, 3);

  // Get newly listed coins (using market cap as a proxy for listing time)
  const newlyListed = [...layerOneCoins]
  ...
  const generateLineChart = (data, isPositive = true) => {
    ...
    return (
      <svg width={width} height={height} className="w-full">
          ...
          return (
            <g key={`point-${i}`}>
              <circle
                ...
                cy={y}
                r="4"
                className="opacity-0 hover:opacity-100 fill-[#2962FF] transition-opacity duration-200"
              />
        ...
        <path
          d={pathString}
          stroke={isPositive ? "#2962FF" : "#ef4444"}
          strokeWidth="2"
          fill="none"
        />
        <path
          ...
          fill={
            isPositive ? "rgba(41, 98, 255, 0.1)" : "rgba(239, 68, 68, 0.1)"
          }
        />
      </svg>
    );
  ...
  const getSortedCoins = () => {
    let filteredCoins = [...layerOneCoins];
    ...
    <div className="container mx-auto bg-white text-gray-800 p-6 rounded-xl">
      <div className="mb-8">
        <h2 className="text-2xl font-bold mb-6 flex items-center">
          Layer 1 Crypto Market Overview
          <span className="bg-gradient-to-r from-[#2962FF] to-[#0D47A1] text-white text-xs px-2 py-1 rounded-full ml-3">
            LIVE
          </span>
        </h2>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-6 rounded-xl shadow-[0_4px_12px_-2px_rgba(0,0,0,0.15)] hover:shadow-[0_6px_16px_-2px_rgba(0,0,0,0.2)] transition-all overflow-hidden bg-white">
            <div className="p-3 font-medium flex items-center justify-between">
              <div className="flex items-center text-[#2962FF]">
          ...
          <div className="col-span-4 rounded-xl shadow-[0_4px_12px_-2px_rgba(0,0,0,0.15)] hover:shadow-[0_6px_16px_-2px_rgba(0,0,0,0.2)] transition-all overflow-hidden bg-white">
            <div className="text-base text-[#2962FF] font-medium flex items-center gap-2 px-4 pt-4 pb-2">
              <TrendingUp size={16} />
              Trending
            </div>
            <div className="">
              {trendingLayerOneCoins.map((coin, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between hover:bg-blue-50 transition-colors group px-4 py-3 cursor-pointer"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="h-6 w-6 rounded-full bg-gradient-to-br from-[#2962FF] to-[#0D47A1] flex items-center justify-center text-white font-bold text-xs shadow-sm flex-shrink-0">
        ...
        <h2 className="text-2xl font-bold mb-4 mt-8">
          Layer 1 Cryptocurrency Prices by Market Cap
        </h2>
        ...
        >
          <div
            className="absolute h-full w-full rounded-full bg-gradient-to-r from-[#2962FF] to-[#0D47A1] transition-all duration-300 ease-in-out"
            style={getTabStyle()}
          />
          {[
            {
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex flex-wrap gap-1 p-1 w-fit">
            {[
              ...
              >
                <span
                  className={`mr-1.5 ${
                    category.id === "layer1" ? "text-blue-600" : "text-gray-500"
                  }`}
                >
                  {category.icon}
                </span>
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="Search Layer 1 coins..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="pl-10 pr-4 py-1.5 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#2962FF] focus:border-transparent text-sm"
            />
            <Search
              size={16}
        ...
        <div className="overflow-hidden rounded-xl">
          <table className="min-w-full table-fixed">
            <thead className="bg-gradient-to-r from-[#2962FF] to-[#0D47A1]">
              <tr>
