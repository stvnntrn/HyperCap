...
const DefiCoins = () => {
  ...
  const getMarketCapRanking = () => {
    const sortedByMarketCap = [...defiCoins].sort(
  ...
  const totalMarketCap = defiCoins.reduce(
    (sum, coin) => sum + coin.marketCap,
    0
  );
  const totalVolume = defiCoins.reduce((sum, coin) => sum + coin.volume, 0);

  // Get trending DeFi coins by volume
  const trendingDefiCoins = [...defiCoins]
    .sort((a, b) => b.volume - a.volume)
    .slice(0, 3);

  // Get top gainers
  const topGainers = [...defiCoins]
    .sort((a, b) => b.change24h - a.change24h)
    .slice(0, 3);

  // Get newly listed coins (using market cap as a proxy for listing time)
  const newlyListed = [...defiCoins]
          ...
          return (
            <g key={`point-${i}`}>
              <circle
                ...
                cy={y}
                r="4"
                className="opacity-0 hover:opacity-100 fill-teal-500 transition-opacity duration-200"
              />
    ...
    <div className="container mx-auto bg-white text-gray-800 p-6 rounded-xl">
      <div className="mb-8">
        <h2 className="text-2xl font-bold mb-6 flex items-center">
          DeFi Crypto Market Overview
          <span className="bg-gradient-to-r from-[#00B4DB] to-[#00F260] text-white text-xs px-2 py-1 rounded-full ml-3">
            LIVE
          </span>
        </h2>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-6 rounded-xl shadow-[0_4px_12px_-2px_rgba(0,0,0,0.15)] hover:shadow-[0_6px_16px_-2px_rgba(0,0,0,0.2)] transition-all overflow-hidden bg-white">
            <div className="p-3 font-medium flex items-center justify-between">
              <div className="flex items-center text-[#00B4DB]">
          ...
          <div className="col-span-4 rounded-xl shadow-[0_4px_12px_-2px_rgba(0,0,0,0.15)] hover:shadow-[0_6px_16px_-2px_rgba(0,0,0,0.2)] transition-all overflow-hidden bg-white">
            <div className="text-base text-[#00B4DB] font-medium flex items-center gap-2 px-4 pt-4 pb-2">
              <TrendingUp size={16} />
              Trending
            </div>
            <div className="">
              {trendingDefiCoins.map((coin, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between hover:bg-[#00B4DB]/10 transition-colors group px-4 py-3 cursor-pointer"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="h-6 w-6 rounded-full bg-gradient-to-br from-[#00B4DB] to-[#00F260] flex items-center justify-center text-white font-bold text-xs shadow-sm flex-shrink-0">
        ...
        <h2 className="text-2xl font-bold mb-4 mt-8">
          DeFi Cryptocurrency Prices by Market Cap
        </h2>
        ...
        >
          <div
            className="absolute h-full w-full rounded-full bg-gradient-to-r from-[#00B4DB] to-[#00F260] transition-all duration-300 ease-in-out"
            style={getTabStyle()}
          />
          {[
            {
        ...
        <div className="flex items-center justify-between mb-2">
            ...
            {[
              ...
              <Link
                ...
                className={`flex items-center px-3 py-1.5 rounded-full text-xs transition-all cursor-pointer font-medium
                  ${
                    category.id === "defi"
                      ? "bg-[#00B4DB]/10 text-[#00B4DB]"
                      : "text-gray-400 hover:text-gray-600"
                  }`}
              >
                <span
                  className={`mr-1.5 ${
                    category.id === "defi" ? "text-[#00B4DB]" : "text-gray-400"
                  }`}
                >
                  {category.icon}
                </span>
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="Search DeFi coins..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="pl-10 pr-4 py-1.5 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#00B4DB] focus:border-transparent text-sm"
            />
            <Search
              size={16}
        ...
        <div className="overflow-hidden rounded-xl">
          <table className="min-w-full table-fixed">
            <thead className="bg-gradient-to-r from-[#00B4DB] to-[#00F260]">
              <tr>
                ...
                <th className="py-4 pl-7 text-left text-xs font-medium text-white uppercase tracking-wider">
                  <div className="flex items-center gap-1">
                    <span
                      className="cursor-pointer hover:text-[#00B4DB] transition-colors"
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {getSortedCoins().map((coin) => {
                ...
                return (
                  <tr
                    key={coin.id}
                    className="hover:bg-[#00B4DB]/10 transition-colors cursor-pointer"
                  >
                    <td className="py-4 pl-3 whitespace-nowrap text-sm font-medium text-gray-700 w-0">
                      <Star
                        size={16}
                        className="text-gray-400 hover:text-[#00B4DB]"
                      />
                    </td>
                    ...
                    <td className="py-4 pl-7 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="h-8 w-8 rounded-full bg-gradient-to-br from-[#00B4DB] to-[#00F260] flex items-center justify-center text-white font-bold mr-3 shadow-md">
                          {coin.symbol.charAt(0)}
                        </div>
