...
async def get_marketcap_data(page: Optional[int] = 1, size: Optional[int] = 100, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "total": {"binance": total_binance, "kraken": total_kraken},
    }
...
async def update_supply_data(db: Session = Depends(get_db)):
    """Update supply data for both Binance and Kraken from CoinGecko."""
    ...
    return {
        ...
        "records": {"binance": binance_records, "kraken": kraken_records},
    }
